...
export function LibraryGrid({ photos, activeId, selectedIds, onOpen, onToggle, onRate }: {
  ...
  return <div className="library-view" aria-label="照片图库">
    <div className="library-heading"><div><p>LIGHTRAW · LIBRARY</p><h1>{photos.length} 张照片</h1></div><span>双击缩略图进入调色</span></div>
    <div className="photo-grid">{photos.map((photo) => <article key={photo.id} className={`photo-card ${activeId === photo.id ? "active" : ""}`}>
      ...
      <div className="photo-meta"><button type="button" onClick={() => onOpen(photo)}><strong title={photo.fileName}>{photo.fileName}</strong>
        ...
        <div className="star-rating" aria-label={`${photo.fileName} 评分`}>{([1, 2, 3, 4, 5] as PhotoRating[]).map((rating) => <button key={rating}
          type="button" className={rating <= photo.rating ? "active" : ""} aria-label={`${rating} 星`}
